...
import { Star, Clock, Paperclip, CheckCircle2, Circle, AlignLeft, Hash } from 'lucide-react';
      ...
      {tasks.map((task) => {
        ...
        return (
          <motion.div
            ...
            className="bg-white/80 dark:bg-[#1C1C1E]/80 backdrop-blur-xl rounded-2xl overflow-hidden shadow-sm border border-gray-100 dark:border-white/5 hover:shadow-2xl transition-all group cursor-pointer flex flex-col"
          >
            <div className="h-32 w-full relative overflow-hidden">
              ...
              {task.lists && (
                <div className={cn(
                  "absolute bottom-3 left-3 flex items-center gap-1.5 px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest backdrop-blur-md border border-white/20",
                  task.lists.color?.replace('text-', 'bg-') || 'bg-white/50',
                  "bg-opacity-20 text-white"
                )}>
                  {ListIcon && <ListIcon className="w-3 h-3" />}
                  {task.lists.name}
                </div>
              )}
            </div>
            ...
            <div className="p-6 flex-1 flex flex-col">
              ...
              <div className="flex flex-wrap gap-1 mb-4">
                {task.tags?.slice(0, 3).map((tag: string) => (
                  <TagBadge key={tag} tag={tag} className="text-[7px] px-1.5 py-0" />
                ))}
              </div>
